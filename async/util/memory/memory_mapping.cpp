...
MemoryMapping::MemoryMapping(char* start, size_t size) noexcept : MemoryMappingView(start, size) {
}

MemoryMapping::MemoryMapping() noexcept : MemoryMappingView() {
}

MemoryMapping::MemoryMapping(MemoryMapping&& other) noexcept : MemoryMapping() {
  Swap(other);
}
...
void MemoryMapping::ProtectPages(size_t first_page, size_t pages, int flags) noexcept {
  assert(mprotect(Data() + first_page * PageSize, pages * PageSize, flags) == 0);
}
